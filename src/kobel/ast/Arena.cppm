...
export namespace kobel::ast {
	...
	struct Arena {
		struct Block {
			...
			explicit Block(const size_t sz) : size(sz), used(0) {
				data = new uint8_t[sz];
			}
		...
		explicit Arena(const size_t bs = 65536) : block_size(bs) {
			blocks.emplace_back(bs);
		}

		void *allocate(const size_t size, const size_t align = 8) {
			...
			if (aligned_used + size > current->size) {
				const size_t new_block_size = block_size > size ? block_size : size;
