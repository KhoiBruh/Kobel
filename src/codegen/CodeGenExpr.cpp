...
llvm::Value *CodeGen::emit_array_literal_expr(const ArrayLiteralExpr *arr_lit) {
	if (!arr_lit) return nullptr;
	auto *lval = emit_lvalue(arr_lit);
	auto sema_ty = get_sema_type(arr_lit);
	auto *arr_type = get_llvm_type(sema_ty);
	return builder->CreateLoad(arr_type, lval, "arr_val");
}

llvm::Value *CodeGen::emit_identifier_expr(const IdentifierExpr *id) {
...
llvm::Value *CodeGen::emit_assign_expr(const AssignExpr *a) {
	if (!a) return nullptr;
...
llvm::Value *CodeGen::emit_binary_expr(const BinaryExpr *b) {
	if (!b) return nullptr;
...
llvm::Value *CodeGen::emit_call_expr(const CallExpr *c) {
	if (!c) return nullptr;
...
llvm::Value *CodeGen::emit_member_expr(const MemberExpr *m) {
	if (!m) return nullptr;
...
llvm::Value *CodeGen::emit_index_expr(const IndexExpr *idx) {
...
llvm::Value *CodeGen::emit_cast_expr(const CastExpr *c) {
	if (!c) return nullptr;
...
llvm::Value *CodeGen::emit_expr(const Expr *expr) {
	...
	if (isa<LiteralExpr>(expr)) return emit_literal_expr(as<LiteralExpr>(expr));
	if (isa<ArrayLiteralExpr>(expr)) return emit_array_literal_expr(as<ArrayLiteralExpr>(expr));
	if (isa<IdentifierExpr>(expr)) return emit_identifier_expr(as<IdentifierExpr>(expr));
	if (isa<AssignExpr>(expr)) return emit_assign_expr(as<AssignExpr>(expr));
	if (isa<BinaryExpr>(expr)) return emit_binary_expr(as<BinaryExpr>(expr));
	if (isa<UnaryExpr>(expr)) return emit_unary_expr(as<UnaryExpr>(expr));
	if (isa<CallExpr>(expr)) return emit_call_expr(as<CallExpr>(expr));
	if (isa<MemberExpr>(expr)) return emit_member_expr(as<MemberExpr>(expr));
	if (isa<IndexExpr>(expr)) return emit_index_expr(as<IndexExpr>(expr));
	if (isa<CastExpr>(expr)) return emit_cast_expr(as<CastExpr>(expr));
...
void CodeGen::emit_when_patterns_branch(
