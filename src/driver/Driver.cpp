...
int Driver::run() {
	...
	if (options_.output_file.empty()) {
		...
		else if (options_.output_file.ends_with(".s") || options_.output_file.ends_with(".asm")) options_.mode = OutputMode::ASSEMBLY;
		else if (options_.output_file.ends_with(".obj") || options_.output_file.ends_with(".o")) options_.mode = OutputMode::OBJECT;
	}
