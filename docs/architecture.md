...
- **AST Builder & Node Downcast**: struct `AstBuilder { arena: &Arena }` (trong `compiler.ast.builder`) đóng gói việc cấp phát toàn bộ các node AST (`b.literal(...)`, `b.binary(...)`, `b.fn_decl(...)`...), loại bỏ tham số `&Arena` lặp lại ở từng lời gọi; `Parser` cung cấp `.b()` để tạo nhanh builder. Toàn bộ phương thức downcast được đưa vào `impl AstNode` (`node.as_literal()`, `node.as_binary()`, `node.as_fn_decl()`...), đồng thời duy trì các hàm độc lập cũ `as_*(node)` và `alloc_*(&arena, ...)` để đảm bảo tương thích ngược 100%.
